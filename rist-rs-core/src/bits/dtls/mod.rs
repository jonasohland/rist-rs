...
use super::super::internal::INTERNAL_ERR_PRE_VALIDATED;
use error::Error;
...
mod test {
    ...
    fn parse_test() {
        ...
        match view.next_layer().unwrap() {
            crate::bits::dtls::DTLSNextLayer::HandshakeProto(h) => {
                println!("{} {:?}", h.message_type(), h.valid_fragment_length());
                println!("{}", h.fragment_offset());
                println!("{}", h.fragment_length());
            }
            crate::bits::dtls::DTLSNextLayer::ChangeCipherSpec() => println!("ccp"),
            crate::bits::dtls::DTLSNextLayer::AlertProto() => println!("alert"),
            crate::bits::dtls::DTLSNextLayer::AppDataProto() => println!("app data"),
        }
    }
}
